...
FORM=`wget -q -c -t 3 --waitretry=3 --user-agent="$USER_AGENT" -O - $URL | sed -n '/download-2/,/64-bit/p'`
...
wget -q -c -t 3 --waitretry=3 --content-disposition --trust-server-names $URLDOWN --post-data "amd_developer_central_nonce=${NONCE2}&f=${FILE}" --user-agent="$USER_AGENT" -O AMD-SDK.tar.bz2;
