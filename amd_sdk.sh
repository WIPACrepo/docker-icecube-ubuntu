...
FORM=`wget -nv -c -t 3 --waitretry=3 --user-agent="$USER_AGENT" -O - $URL | sed -n '/download-2/,/64-bit/p'`
...
FORM=`wget -nv -c -t 3 --waitretry=3 --user-agent="$USER_AGENT" -O - $URLDOWN --post-data "amd_developer_central_downloads_page_nonce=${NONCE1}&f=${FILE}&post_id=${POSTID}"`

NONCE2=`echo $FORM | awk -F ${NONCE2_STRING} '{print $2}'`
NONCE2=`echo $NONCE2 | awk -F'"' '{print $2}'`

wget -nv -c -t 3 --waitretry=3 --content-disposition --trust-server-names $URLDOWN --post-data "amd_developer_central_nonce=${NONCE2}&f=${FILE}" --user-agent="$USER_AGENT" -O AMD-SDK.tar.bz2;
